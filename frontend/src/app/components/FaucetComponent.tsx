...
const GELATO_API_KEY = "EPJwebwW8P_7ZGLJYr6GOd8HvZnfP7hMKTLA6xP_TX4_";
...
const FaucetComponent: React.FC<FaucetComponentProps> = ({ onSuccess }) => {
  ...
  // Read TokenA Balance
  const { data: tokenABalanceData, refetch: refetchTokenABalance } =
    useReadContract({
  ...
  // Read TokenB Balance
  const { data: tokenBBalanceData, refetch: refetchTokenBBalance } =
    useReadContract({
  ...
  const switchToABCTestnet = useCallback(async () => {
    if (!switchChain) {
      setStatusMessage(
        "Cannot switch network. Please use your wallet's network selector."
      );
      return;
    }
  ...
  const startPollingForTaskStatus = useCallback(
    async (taskId: string) => {
        ...
        try {
          ...
          if (status && status.taskState) {
            ...
            if (status.taskState === TaskState.ExecSuccess) {
              ...
              // Transaction failed
              setStatusMessage(
                `Transaction failed with status: ${status.taskState}`
              );
              setIsRelaying(false);
              return;
            }
            ...
            setStatusMessage(
              "Error getting task status: Status response is empty"
            );
            setIsRelaying(false);
          }
        } catch (error) {
  ...
  const handleGaslessRequestTokens = useCallback(async () => {
    ...
    try {
      ...
      const faucet = CONTRACT_ADDRESSES.FAUCET;
      const abi = ["function requestFaucet()"];
      const provider = new ethers.BrowserProvider(window.ethereum);
      const signer = await provider.getSigner();
      const user = await signer.getAddress();

      const contract = new ethers.Contract(faucet, abi, signer);
      const { data } = await contract.requestFaucet.populateTransaction();
    ...
    <div className="p-4 rounded-xl bg-gray-700 max-w-2xl mx-auto">
      ...
      {!isCorrectNetwork && (
        <div className="mb-4 p-3 bg-red-900/50 text-red-200 rounded-lg">
          <p>
            You need to connect to ABC Testnet (Chain ID: 112) to use this
            faucet.
          </p>
      ...
      {!isConnected ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              {isRequestingFaucet || isConfirming
                ? "Requesting..."
                : "Request Tokens"}
            </button>
      ...
      <div className="mt-4 text-sm text-gray-400">
        <p>
          The faucet allows you to obtain free TokenA and TokenB to test the
          Gorillix DEX platform.
        </p>
        <p className="mt-2">
          <span className="text-violet-400">Standard request:</span> Requires
          gas fees for the transaction.
        </p>
        <p>
          <span className="text-yellow-400">Gasless request:</span> No gas fees
          required (sponsored by Gelato Relay).
        </p>
      </div>
    </div>
  );
