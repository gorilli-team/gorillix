...
import { gorillixActionProvider } from "../../gorillix/gorillixActionProvider";
...
const myCustomChain = defineChain({
  id: 112, // ID della custom chain
  name: "ABC Testnet", // Nome della custom chain
  network: "ABC Testnet", // Tipo di rete (personalizzato)
  nativeCurrency: {
    name: "TEST",
    symbol: "TEST",
    decimals: 18,
  ...
  rpcUrls: {
    default: {
      http: ["https://rpc.abc.t.raas.gelato.cloud"], // Sostituisci con l'URL RPC della tua custom chain
    },
  },
  blockExplorers: {
    default: {
      name: "abc explorer",
      url: "https://explorer.abc.t.raas.gelato.cloud/", // Sostituisci con l'URL dell'explorer della tua custom chain
    },
  },
});
...
async function getOrInitializeAgent(): Promise<ReturnType<typeof createReactAgent>> {
  ...
  try {
    ...
    const walletProvider = await new ViemWalletProvider(walletClient);
